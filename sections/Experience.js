...
export default function Experience() {
  const [openCompany, setOpenCompany] = useState(experience[0]?.company || "");
              ...
              <SectionReveal key={item.company} className="relative pl-8 md:pl-12">
                ...
                <article className="glass rounded-[8px] p-6 transition hover:border-cyan/40 md:p-8">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setOpenCompany(isOpen ? "" : item.company)}
                      className="w-fit rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-white/62 transition hover:border-cyan/50 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan"
                  ...
                  <ul className="mt-7 grid gap-3 md:grid-cols-2">
                    {(isOpen ? item.responsibilities : item.responsibilities.slice(0, 4)).map((responsibility) => (
                  ...
                  {item.responsibilities.length > 4 ? (
                    <button
                      type="button"
                      onClick={() => setOpenCompany(isOpen ? "" : item.company)}
                      className="mt-5 text-sm font-semibold text-cyan transition hover:text-white focus:outline-none focus-visible:rounded focus-visible:ring-2 focus-visible:ring-cyan"
                      aria-expanded={isOpen}
                    >
                      {isOpen ? "Show less" : `Show ${item.responsibilities.length - 4} more details`}
                    </button>
                  ) : null}
                </article>
              </SectionReveal>
